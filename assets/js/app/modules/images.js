...
appMakeBeCool.gateway.addClass('Images', function(properties, $, $window, $document) {
    ...
    //PRIVATE METHODS
    _init = function() {
        appMakeBeCool.gateway.base.Class.apply(_images, [_p]);
        if(!_g.preloaded) {
            return _images.init();
        }
    ...
    _generateImgs = function (options) {
        ...
        $context.find('['+_p.imgDataSrc+']').each(function () {
            var $this = $(this);
            ...
            if ($this.hasClass(_p.imgPreloadClass) && imgsrc) {
            ...
            $.imgpreload(imagesSrc, {
                each: function (i) {
                    ...
                    setTimeout(function () {
                        ...
                        $(img).attr(_p.imgDataSrc, '');
                        $(img).attr(_p.imgDataSrc2x, '');
                    }, _p.simulatedDelay)
                },
                all: function(){
    ...
    _makeImageResponsive = function(img){
        ...
        ////////LOOP ALL IMAGES////////
        //set attr names
        var srcAttr = ( retina && hasAttr(image, _p.imgDataSrc2x) ) ? _p.imgDataSrc2x : _p.imgDataSrc;
    ...
    _addImgPreloader = function (imgEl) {
        ...
        if ($imgEl.hasClass(_p.imgNoPreloadClass)) return console.log('No add preload');
        var parent = $imgEl.parent('figure');
        if (parent.children(_p.loaderClass).length === 1) return;
        parent.append(_p.loaderTemplate);
        $imgEl.hide();
    },
    ...
    _removeImgPreloader = function (imgEl) {
        ...
        var $preload = parent.children(_p.loaderClass);
